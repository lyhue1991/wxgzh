...
- 支持在同一套用户配置中管理多个微信公众号账号，并在发布时按文章或命令选择目标账号。
...
- 根命令会直接调用微信接口，因此要求已选中的账号已配置 `appid` 与 `appsecret`。
- 账号选择顺序为：命令行 `--account` > front matter `account` > 环境变量 `WX_ACCOUNT` > 用户配置中的默认账号。
...
- `--account <name>`：指定要查看或修改的账号名
- `--appid <appid>`：为目标账号设置微信公众号 appid
- `--appsecret <appsecret>`：为目标账号设置微信公众号 appsecret
- `--author <author>`：为目标账号设置默认作者名
- `--default-theme <theme>`：为目标账号设置默认主题
- `--enable-comment`：为目标账号默认开启评论
- `--disable-comment`：为目标账号默认关闭评论
- `--set-default-account <name>`：设置默认账号
- `--remove-account <name>`：删除指定账号配置
- `--list`：查看当前账号或指定账号的生效配置
- `--list-accounts`：列出全部已配置账号
- `--list-themes`：列出可用主题
- `--clear`：清空用户级配置文件
...
wxgzh config --account 公众号名称2 --author "技术团队" --default-theme blue
wxgzh config --set-default-account 公众号名称1
wxgzh config --list-themes
wxgzh config --account 公众号名称1 --enable-comment
```

说明：

- 用户级配置文件路径为 `~/.config/wxgzh/wxgzh.json`。
- 配置文件支持多个账号，账号名由用户自定义，例如 `公众号名称1`、`公众号名称2`、`team-a`。
- **配置凭据时必须指定账号**：设置 `appid` 或 `appsecret` 时，必须同时通过 `--account` 指定目标账号名。
- 当执行写入类操作且未传 `--account` 时，默认作用于当前默认账号；若尚未设置默认账号，则首次新增账号时必须显式传入 `--account`。
- `config --list` 默认展示当前默认账号的生效配置；若同时传入 `--account`，则展示对应账号。
- 当前未实现项目级配置文件。
- 环境变量支持 `WX_ACCOUNT`、`WX_APPID`、`WX_APPSECRET`。
...
- 支持 front matter：`title`、`author`、`digest`、`theme`、`cover`、`enableComment`、`account`。
...
- `--account <name>`：指定上传正文图片时使用的公众号账号名
...
- 若需要上传图片，账号选择顺序同根命令：`--account` > HTML 元数据中的 `account` > `WX_ACCOUNT` > 默认账号。
...
- 账号选择顺序为：命令行 `--account` > HTML 元数据中的 `account` > 环境变量 `WX_ACCOUNT` > 用户配置中的默认账号。
...
- `digest`：摘要
- `theme`：主题名
- `account`：目标公众号账号名，用于选择配置文件中的某个账号
- `cover`：封面图相对或绝对路径
...
- `currentAccount` 表示默认账号名。
- `accounts` 以账号名为 key，保存每个公众号各自的凭据与默认发布参数。
- `config --list` 输出的是当前账号或指定账号的生效配置。
- 若同时设置 `WX_APPID` 与 `WX_APPSECRET`，则它们会覆盖所选账号中的凭据，仅对当前进程生效。
- `appsecret` 在输出时会自动脱敏。
- 微信 access token 会额外缓存到 `~/.config/wxgzh/token.<appid>.json`。

### 账号解析规则

- 需要访问微信接口的命令包括根命令、`fix`（上传图片时）与 `publish`。
- 账号名优先级：命令行 `--account` > front matter / HTML 元数据中的 `account` > 环境变量 `WX_ACCOUNT` > `currentAccount`。
- 账号凭据来源：若环境变量同时提供 `WX_APPID` 与 `WX_APPSECRET`，优先使用环境变量；否则读取已选中账号的配置。
- 若最终仍无法确定账号或缺少凭据，命令应报错，并给出明确提示，例如“未找到公众号账号 公众号名称1 的 appsecret 配置”。
...
| 包名 | 用途 |
|------|------|
| `@css-inline/css-inline` | CSS 内联到 HTML |
| `axios` | 微信接口与远程资源请求 |
| `cheerio` | HTML 结构处理 |
| `commander` | CLI 框架 |
| `dotenv` | 环境变量加载 |
...
- 使用 `@css-inline/css-inline` 把主题 CSS 与补充样式内联到最终 HTML。
